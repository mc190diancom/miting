...
public class FaceManager {
    ...
    public static void handlerEmojiText2(TextView comment, String content, int length,boolean typing) {
        SpannableStringBuilder sb = new SpannableStringBuilder(content);
        String regex = "\\[(\\S+?)\\]";
        Pattern p = Pattern.compile(regex);
        Matcher m = p.matcher(content);
        try {
            sb.setSpan(new ForegroundColorSpan(Color.parseColor("#3B73E1")),0,length+1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }catch (Exception e){
