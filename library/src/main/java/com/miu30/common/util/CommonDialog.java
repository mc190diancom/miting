...
public class CommonDialog {
	...
	/*public CommonDialog(Context ctx,boolean sheet,OnResultListener ) {
	...
	public CommonDialog(Context ctx,boolean sheet,CommonDialog.OnResultListener listener) {
		dialog = getDialog(ctx,listener);
		if (sheet) {
			Window window = dialog.getWindow();
			LayoutParams param = window.getAttributes();
			param.gravity = Gravity.CENTER;
	...
	public static Dialog getDialog(Context ctx, final CommonDialog.OnResultListener listener) {
		/*int layout = R.layout.layout_live;
		int style = R.style.clean_dialog;
		final Dialog dialog = new Dialog(ctx, style);
		dialog.setContentView(layout);
		dialog.setCanceledOnTouchOutside(true);
		Button okBtn = dialog.findViewById(R.id.btn_save);
		final EditText etStr = dialog.findViewById(R.id.et_live);
		okBtn.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				listener.onResultStr(etStr.getText().toString());
				dialog.dismiss();
			}
		});*/
