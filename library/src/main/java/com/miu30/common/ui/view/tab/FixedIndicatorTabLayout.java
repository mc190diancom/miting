 ...
 */
@ViewPager.DecorView
public class FixedIndicatorTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link FixedIndicatorTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link FixedIndicatorTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    public FixedIndicatorTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FixedIndicatorTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.FixedIndicatorTabLayout,
                defStyleAttr, R.style.Widget_Design_TabLayout);

        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabIndicatorHeight, 0));
        // default value was 28dp
        mTabStrip.setSelectedIndicatorWidth(
                a.getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabIndicatorWidth, dpToPx(28)));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.FixedIndicatorTabLayout_tabIndicatorColor, 0));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabPaddingBottom,
                mTabPaddingBottom);

        mTabTextAppearance = a.getResourceId(R.styleable.FixedIndicatorTabLayout_tabTextAppearance,
        ...
        if (a.hasValue(R.styleable.FixedIndicatorTabLayout_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.FixedIndicatorTabLayout_tabTextColor);
        }

        if (a.hasValue(R.styleable.FixedIndicatorTabLayout_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.FixedIndicatorTabLayout_tabSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors.getDefaultColor(), selected);
        }

        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.FixedIndicatorTabLayout_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.FixedIndicatorTabLayout_tabContentStart, 0);
        mMode = a.getInt(R.styleable.FixedIndicatorTabLayout_tab_mode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.FixedIndicatorTabLayout_tab_gravity, GRAVITY_FILL);
        a.recycle();
    ...
    /**
     * Add a {@link FixedIndicatorTabLayout.OnTabSelectedListener} that will be invoked when tab selection
     * changes.
     * <p>
    ...
    /**
     * Remove the given {@link FixedIndicatorTabLayout.OnTabSelectedListener} that was previously added via
    ...
    /**
     * Remove all previously added {@link FixedIndicatorTabLayout.OnTabSelectedListener}s.
     */
    ...
    /**
     * Returns the current mode used by this {@link FixedIndicatorTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link FixedIndicatorTabLayout} with a {@link ViewPager}.
     * <p>
    ...
    public static final class Tab {
        ...
        FixedIndicatorTabLayout mParent;
        TabView mView;
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        ...
        // We return the default layout params so that we don't blow up if we're given a CustomTabItem
     ...
     * necessary calls back to the provided {@link FixedIndicatorTabLayout} so that the tab position is
     * kept in sync.
     * <p>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<FixedIndicatorTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(FixedIndicatorTabLayout tabLayout) {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final FixedIndicatorTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
    ...
    /**
     * A {@link FixedIndicatorTabLayout.OnTabSelectedListener} class which contains the necessary calls back
     * to the provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements FixedIndicatorTabLayout.OnTabSelectedListener {
        ...
        @Override
        public void onTabSelected(FixedIndicatorTabLayout.Tab tab) {
            mViewPager.setCurrentItem(tab.getPosition());
        }

        @Override
        public void onTabUnselected(FixedIndicatorTabLayout.Tab tab) {
            // No-op
        }

        @Override
        public void onTabReselected(FixedIndicatorTabLayout.Tab tab) {
            // No-op
        }
    }
